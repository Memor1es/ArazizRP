...
AddEventHandler('esx_SodaMachine:Random', function(prop_name)
	...
	if lottery <= 6 then
		if not IsAnimated then
			...
			Citizen.CreateThread(function()
				...
				TriggerServerEvent('esx_SodaMachine:TakeMoney', 7)
				Citizen.Wait(3000)
				...
				Citizen.Wait(700)
				ESX.ShowNotification('Vous avez acheté une canette de ~r~Coca Cola~s~ pour ~g~7$~s~.')
	...
	if lottery == 7 then
			Citizen.CreateThread(function()
			ESX.ShowNotification('La machine est indisponible')
			end)
		end
	if lottery == 8 then
			Citizen.CreateThread(function()
			ESX.ShowNotification('La machine est en rupture de stock')
			end)
		end
	if lottery == 9 then
			Citizen.CreateThread(function()
			...
			TriggerEvent('InteractSound_CL:PlayOnOne', 'SodaMachine', 1.0)
			Citizen.Wait(5000)
			ESX.ShowNotification('La machine est restée bloquée et à gardé votre argent')
			FreezeEntityPosition(ped, false)
			end)
		end
end)