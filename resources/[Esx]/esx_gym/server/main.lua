...
AddEventHandler('esx_gym:hireBmx', function()
	...
	if(xPlayer.getMoney() >= 250) then
		...
		notification("Vous avez volé un BMX car vous n'aviez pas assez ~r~d'argent")
	end	
end)
...
AddEventHandler('esx_gym:hireCruiser', function()
	...
	if(xPlayer.getMoney() >= 300) then
		...
		notification("Vous avez volé un CRUISER car vous n'aviez pas assez ~r~d'argent")
	end	
end)
...
AddEventHandler('esx_gym:hireFixter', function()
	...
	if(xPlayer.getMoney() >= 329) then
		...
		notification("Vous avez volé un FIXTER car vous n'aviez pas assez ~r~d'argent")
	end	
end)
...
AddEventHandler('esx_gym:hireScorcher', function()
	...
	if(xPlayer.getMoney() >= 400) then
		...
		notification("Vous avez volé un SCORCHER car vous n'aviez pas assez ~r~d'argent")
	end	
end)
...
AddEventHandler('esx_gym:buyMembership', function()
	...
	if(xPlayer.getMoney() >= 800) then
		...
		notification("Vous avez acheté un abonnement à la ~g~salle de sport")
...
AddEventHandler('esx_gym:buyProteinshake', function()
	...
	if(xPlayer.getMoney() >= 6) then
		...
		notification("Vous avez acheté une ~g~boisson vitaminée")
	else
		notification("Vous n'avez pas assez ~r~d'argent")
	end	
end)
...
ESX.RegisterUsableItem('protein_shake', function(source)
	...
	TriggerClientEvent('esx:showNotification', source, 'Vous avez utilisé une a ~g~boisson vitaminée')
...
AddEventHandler('esx_gym:buySportlunch', function()
	...
	if(xPlayer.getMoney() >= 2) then
		...
		notification("Vous avez acheté un ~g~repas équilibré")
	else
		notification("Vous n'avez pas assez ~r~d'argent")
	end		
end)
...
ESX.RegisterUsableItem('sportlunch', function(source)
	...
	TriggerClientEvent('esx:showNotification', source, 'Vous avez mangé un ~g~repas équilibré')
...
AddEventHandler('esx_gym:buyPowerade', function()
	...
	if(xPlayer.getMoney() >= 4) then
		...
		notification("Vous avez acheté une bouteille de ~g~powerade")
	else
		notification("Vous n'avez pas assez ~r~d'argent")
	end		
end)
...
ESX.RegisterUsableItem('powerade', function(source)
	...
	TriggerClientEvent('esx_basicneeds:onDrink', source)
	TriggerClientEvent('esx:showNotification', source, 'Vous avez utilisé une bouteille de ~g~powerade')
