...
local blips = {
	{title="Salle de sport", colour=7, id=311, x = -1201.2257, y = -1568.8670, z = 4.6101}
}
            ...
            if dist <= 0.5 then
				...
				if IsControlJustPressed(0, Keys['E']) then -- "E"
					if IsPedInAnyVehicle(GetPlayerPed(-1)) then
						ESX.ShowNotification("Vous avez déjà un ~r~véhicule")
					else
						OpenBikeMenu()
					end
				end			
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(gym) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyez sur ~INPUT_CONTEXT~ pour ouvrir le menu de ~b~gym')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(arms) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyez sur ~INPUT_CONTEXT~ pour faire travailler vos ~g~bras')
				
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
					
						TriggerServerEvent('esx_gym:checkChip')
						ESX.ShowNotification("Preparation de l'~g~exercice~w~...")
						Citizen.Wait(1000)					
						...
						if membership == true then
							...
							ESX.ShowNotification("Vous devez vous reposer ~r~60 secondes ~w~avant de refaire un exercice")
							
							--TriggerServerEvent('esx_gym:trainArms') ## COMING SOON...
							
							training = true
						elseif membership == false then
							ESX.ShowNotification("Il vous faut un abonnement à la salle de sport")
						end
...
function CheckTraining()
	...
	if resting == false then
		ESX.ShowNotification("Vous pouvez retourner faire vos exercices...")
	end
end
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_menu',
        {
            ...
            elements = {
				...
				{label = 'Horaires d\'ouvertures', value = 'hours'},
...
function OpenGymShopMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_shop_menu',
        {
            title    = 'Gym - Shop',
            elements = {
				{label = 'Boissin vitaminée ($6)', value = 'protein_shake'},
				{label = 'Eau ($1)', value = 'water'},
				{label = 'Repas équilibré ($2)', value = 'sportlunch'},
...
function OpenGymShipMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_ship_menu',
        {
            ...
            elements = {
				{label = 'Carte d\'abonnement ($800)', value = 'membership'},
            }
        },
        function(data, menu)
