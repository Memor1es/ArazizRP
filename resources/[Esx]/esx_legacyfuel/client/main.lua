		...
		if not InBlacklistedVehicle then
			...
			if nearPump and IsCloseToLastVehicle then
				...
				if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Quitter votre véhicule pour faire le plein")
				elseif IsFueling then
					local position = GetEntityCoords(vehicle)

					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Appuyez sur ~g~G ~w~to pour arrêter de faire le plein. $~r~" .. price .. " ~w~ + taxes")
				...
				elseif fuel > 95.0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Le véhile contient trop d'essence pour faire le plein")
				elseif cash <= 0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Vous n'avez pas assez d'argent pour faire le plein")
				else
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Appuyez sur ~g~G ~w~to pour faire le plein. $~r~0.7/~w~litres + taxes")
			...
			elseif NearVehicleWithJerryCan and not nearPump and Config.EnableJerryCans then
				...
				if IsFuelingWithJerryCan then
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Appuyez sur ~g~G ~w~pour arrêter de faire le plein. Réservoir: " .. fuel .. "% - Bidon: " .. jerrycan)
					...
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Appuyez sur ~g~G ~w~pour mettre de l'essence avec un bidon")
...
function DisplayHud()
	...
	if Config.ShouldDisplayHud and IsPedInAnyVehicle(playerPed, false) and GetSeatPedIsIn(playerPed) == -1 then
		...
		DrawAdvancedText(0.120 - x, 0.73 - y, 0.005, 0.0028, 0.6, fuel, 255, 255, 255, 255, 6, 1)
		
		--DrawAdvancedText(0.2195 - x, 0.77 - y, 0.005, 0.0028, 0.6, fuel, 255, 255, 255, 255, 6, 1)
		--DrawAdvancedText(0.130 - x, 0.77 - y, 0.005, 0.0028, 0.6, mph, 255, 255, 255, 255, 6, 1)
		--DrawAdvancedText(0.174 - x, 0.77 - y, 0.005, 0.0028, 0.6, kmh, 255, 255, 255, 255, 6, 1)

		DrawAdvancedText(0.139 - x, 0.7365 - y, 0.005, 0.0028, 0.4, "Litres d'essence", 255, 255, 255, 255, 6, 1)
	end
end
