...
function OpenInfoIllegalMenu()
  ...
  local elements = { }
	  ...
	  table.insert(elements, {label = _U('blanche') .. Config.PriceBlanc .. _U('blanche1'),    value = 'blanche'})
	  table.insert(elements, {label = _U('armes') .. Config.PriceArmes .. _U('armes1'),    value = 'armes'})
  ...
  ESX.UI.Menu.CloseAll()
    ...
    ESX.UI.Menu.Open(
      'default', GetCurrentResourceName(), 'info',
      {
		...
		if data.current.value == 'blanche' then
           TriggerServerEvent("esx_infoillegal:Blanche")
		   ESX.UI.Menu.CloseAll()
        end
		if data.current.value == 'armes' then
           TriggerServerEvent("esx_infoillegal:Armes")
		   ESX.UI.Menu.CloseAll()
        end
...
RegisterNetEvent("esx_infoillegal:Blanchiment")
AddEventHandler("esx_infoillegal:Blanchiment", function()
	if Config.GPS then
		x, y, z = Config.Blanchiment.x, Config.Blanchiment.y, Config.Blanchiment.z
		SetNewWaypoint(x, y, z)
		local source = GetPlayerServerId();
		TriggerEvent("esx_infoillegal:notify", "CHAR_LESTER_DEATHWISH", 1, _U('Notification'), false, _U('GPS'))
	else
		TriggerEvent("esx_infoillegal:notify", "CHAR_LESTER_DEATHWISH", 1, _U('Notification'), false, _U('Blanche'))
		TriggerEvent("esx_infoillegal:notify", "CHAR_LESTER_DEATHWISH", 1, _U('Notification'), false, _U('Blanche1'))
	end
end)

RegisterNetEvent("esx_infoillegal:ArmesIllegales")
AddEventHandler("esx_infoillegal:ArmesIllegales", function()
	if Config.GPS then
		x, y, z = Config.ArmesIllegales.x, Config.ArmesIllegales.y, Config.ArmesIllegales.z
		SetNewWaypoint(x, y, z)
		local source = GetPlayerServerId();
		TriggerEvent("esx_infoillegal:notify", "CHAR_LESTER_DEATHWISH", 1, _U('Notification'), false, _U('GPS'))
	else
		TriggerEvent("esx_infoillegal:notify", "CHAR_LESTER_DEATHWISH", 1, _U('Notification'), false, _U('Armes'))
		TriggerEvent("esx_infoillegal:notify", "CHAR_LESTER_DEATHWISH", 1, _U('Notification'), false, _U('Armes1'))
	end
end)
